...
var FlacParser = /** @class */ (function (_super) {
    ...
    FlacParser.prototype._parse = function () {
        var _this = this;
        return this.tokenizer.readToken(FourCC_1.FourCcToken).then(function (fourCC) {
            if (fourCC.toString() !== 'fLaC') {
                throw new Error("Invalid FLAC preamble");
            }
            return _this.parseBlockHeader().then(function () {
                if (_this.tokenizer.fileSize && _this.metadata.format.duration) {
                    var dataSize = _this.tokenizer.fileSize - _this.tokenizer.position;
                    _this.metadata.setFormat('bitrate', 8 * dataSize / _this.metadata.format.duration);
                }
            });
        });
    };
    ...
    FlacParser.prototype.addTag = function (id, value) {
    ...
    FlacParser.prototype.parseBlockStreamInfo = function (dataLen) {
        ...
        return this.tokenizer.readToken(Metadata.BlockStreamInfo).then(function (streamInfo) {
            _this.metadata.setFormat('dataformat', 'flac');
            _this.metadata.setFormat('lossless', true);
            _this.metadata.setFormat('numberOfChannels', streamInfo.channels);
            _this.metadata.setFormat('bitsPerSample', streamInfo.bitsPerSample);
            _this.metadata.setFormat('sampleRate', streamInfo.sampleRate);
            _this.metadata.setFormat('duration', streamInfo.totalSamples / streamInfo.sampleRate);
        });
    };
    /**
    ...
    FlacParser.prototype.parseComment = function (dataLen) {
        ...
        return this.tokenizer.readToken(new Token.BufferType(dataLen)).then(function (data) {
            ...
            for (var i = 0; i < commentListLength; i++) {
                ...
                _this.addTag(split[0].toUpperCase(), split.splice(1).join('='));
            }
        });
    };
    FlacParser.prototype.parsePicture = function (dataLen) {
        ...
        if (this.options.skipCovers) {
            ...
            return this.tokenizer.readToken(new Vorbis_1.VorbisPictureToken(dataLen)).then(function (picture) {
                _this.addTag('METADATA_BLOCK_PICTURE', picture);
            });
        }
    };
    return FlacParser;
