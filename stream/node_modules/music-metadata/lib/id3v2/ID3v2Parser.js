...
var ID3v2Parser = /** @class */ (function () {
    ...
    ID3v2Parser.prototype.parse = function (metadata, tokenizer, options) {
        var _this = this;
            ...
            for (var _i = 0, _a = _this.parseMetadata(buffer); _i < _a.length; _i++) {
                ...
                if (tag.id === 'TXXX') {
                    for (var _b = 0, _c = tag.value.text; _b < _c.length; _b++) {
                        var text = _c[_b];
                        _this.addTag(ID3v2Parser.makeDescriptionTagName(tag.id, tag.value.description), text);
                    }
                }
                else if (tag.id === 'COM') {
                    for (var _d = 0, _e = tag.value; _d < _e.length; _d++) {
                        var value = _e[_d];
                        _this.addTag(ID3v2Parser.makeDescriptionTagName(tag.id, value.description), value.text);
                    }
                }
                else if (util_1.isArray(tag.value)) {
                    for (var _f = 0, _g = tag.value; _f < _g.length; _f++) {
                        var value = _g[_f];
                        _this.addTag(tag.id, value);
                    }
                }
                else {
                    _this.addTag(tag.id, tag.value);
                }
            }
        });
    };
    ID3v2Parser.prototype.addTag = function (id, value) {
