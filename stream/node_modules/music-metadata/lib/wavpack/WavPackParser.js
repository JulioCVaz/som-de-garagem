 ...
 * WavPack Parser
 */
var WavPackParser = /** @class */ (function (_super) {
    __extends(WavPackParser, _super);
    function WavPackParser() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    WavPackParser.prototype.parse = function () {
        var _this = this;
        // First parse all WavPack blocks
        return this.parseWavPackBlocks()
            .then(function () {
            // try to parse APEv2 header
            return APEv2Parser_1.APEv2Parser.parseFooter(_this.metadata, _this.tokenizer, _this.options);
        });
    };
        ...
        return this.tokenizer.peekToken(FourCC_1.FourCcToken).then(function (blockId) {
            if (blockId === 'wvpk') {
                return _this.tokenizer.readToken(WavPack.BlockHeaderToken)
                    ...
                    if (header.blockIndex === 0 && !_this.metadata.format.dataformat) {
                        _this.metadata.setFormat('dataformat', 'WavPack');
                        _this.metadata.setFormat('lossless', !header.flags.isHybrid);
                        // tagTypes: this.type,
                        _this.metadata.setFormat('bitsPerSample', header.flags.bitsPerSample);
                        _this.metadata.setFormat('sampleRate', header.flags.samplingRate);
                        _this.metadata.setFormat('numberOfChannels', header.flags.isMono ? 1 : 2);
                        _this.metadata.setFormat('duration', header.totalSamples / header.flags.samplingRate);
                    }
                ...
                return _this.tokenizer.readBuffer(data, 0, data.length).then(function () {
                    switch (id.functionId) {
                        ...
                        case 0x26: // ID_MD5_CHECKSUM
                            _this.metadata.setFormat('audioMD5', data);
                            break;
