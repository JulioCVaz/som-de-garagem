...
var ID3v1Parser = /** @class */ (function (_super) {
    __extends(ID3v1Parser, _super);
    ...
    ID3v1Parser.prototype.parse = function () {
        var _this = this;
        if (!this.tokenizer.fileSize) {
            debug('Skip checking for ID3v1 because the file-size is unknown');
        }
        return this.tokenizer.readToken(Iid3v1Token, this.tokenizer.fileSize - Iid3v1Token.len).then(function (header) {
            if (header) {
                debug("ID3v1 header found at: pos=%s", _this.tokenizer.fileSize - Iid3v1Token.len);
                var id3 = [];
                for (var _i = 0, _a = ["title", "artist", "album", "comment", "track", "year"]; _i < _a.length; _i++) {
                    var id = _a[_i];
                    if (header[id] && header[id] !== "")
                        _this.addTag(id, header[id]);
                }
                var genre = ID3v1Parser.getGenre(header.genre);
                if (genre)
                    _this.addTag('genre', genre);
            }
            else {
                debug("ID3v1 header not found at: pos=%s", _this.tokenizer.fileSize - Iid3v1Token.len);
            }
        }).then();
    };
    ID3v1Parser.prototype.addTag = function (id, value) {
