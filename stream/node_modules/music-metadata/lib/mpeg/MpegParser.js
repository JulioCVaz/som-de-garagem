...
var MpegParser = /** @class */ (function (_super) {
     ...
     */
    MpegParser.prototype.finalize = function () {
        ...
        if (!format.duration && this.tokenizer.fileSize && format.codecProfile === "CBR") {
            ...
            var numberOfSamples = Math.round(mpegSize / this.frame_size) * this.samplesPerFrame;
    ...
    MpegParser.prototype.parseAudioFrameHeader = function (buf_frame_header) {
        ...
        return this.tokenizer.readBuffer(buf_frame_header, 1, 3).then(function () {
            ...
            _this.metadata.setFormat('dataformat', 'mp' + header.layer);
            _this.metadata.setFormat('lossless', false);
            _this.metadata.setFormat('bitrate', header.bitrate);
            _this.metadata.setFormat('sampleRate', header.samplingRate);
            _this.metadata.setFormat('numberOfChannels', header.channelMode === "mono" ? 1 : 2);
            ...
            if (_this.frameCount === 3) {
                ...
                if (_this.areAllSame(_this.bitrates)) {
                    ...
                    _this.metadata.setFormat('codecProfile', 'CBR');
                    if (_this.tokenizer.fileSize)
                        return; // Calculate duration based on file size
                }
                else if (!_this.options.duration) {
                    return; // Done
                }
            }
            ...
            if (_this.options.duration && _this.frameCount === 4) {
                _this.samplesPerFrame = samples_per_frame;
            ...
            switch (headerTag) {
                ...
                case "Xing":
                    return _this.readXingInfoHeader().then(function (infoTag) {
                        var codecProfile = MpegAudioLayer.getVbrCodecProfile(infoTag.vbrScale);
                        _this.metadata.setFormat('codecProfile', codecProfile);
                        return null;
                    });
                case "Xtra":
                    // ToDo: ???
                ...
                case "LAME":
                    return _this.tokenizer.readToken(XingTag_1.LameEncoderVersion).then(function (version) {
                        ...
                        _this.metadata.setFormat('encoder', 'LAME ' + version);
    ...
    MpegParser.prototype.readXingInfoHeader = function () {
        ...
        return this.tokenizer.readToken(XingTag_1.XingInfoTag).then(function (infoTag) {
            ...
            _this.metadata.setFormat('encoder', Util_1.default.stripNulls(infoTag.encoder));
            if ((infoTag.headerFlags[3] & 0x01) === 1) {
                var duration = _this.audioFrameHeader.calcDuration(infoTag.numFrames);
                _this.metadata.setFormat('duration', duration);
